...
results_save_path = results_save_path+str(datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S')) + '-result_score.txt'
...
for method_name, method_map_root in RGBD_SOD_Models.items():
    ...
    for name, root in test_datasets.items():
        ...
        if os.path.exists(sal_root):
            ...
            ber = ber.show()
            acc = acc.show()
            log = 'method_name: {} dataset: {} MAE: {:.4f} Ber: {:.4f} maxF: {:.4f} avgF: {:.4f} wfm: {:.4f} Sm: {:.4f} Em: {:.4f} M_dice: {:.4f} M_iou: {:.4f} Acc: {:.4f}'.format(
                    method_name, name, MAE, ber, maxf, meanf, wfm, sm, em, m_dice, m_iou, acc)
            print('\n'+log)
            open(results_save_path, 'a').write(log+'\n')
